...
color_info = message_filters.Subscriber("/camera/aligned_depth_to_color/camera_info", CameraInfo)
...
# Load the model
interpreter = tf.lite.Interpreter(model_path='/home/turtlebot/catkin_ws/src/vision_leg_tracker/src/3.tflite')
...
def depth_scan_callback(depth, color, color_info, scan):
    ...
    try: 
        ...
        # person_pixel = np.array(depth_array.shape) / 2 # fake pixel at the center of the image (row, column)
        ...
        print(e)    
def main():
    ats = message_filters.ApproximateTimeSynchronizer([depth_sub, color_sub, color_info, scan_sub], queue_size=10, slop=0.1)
