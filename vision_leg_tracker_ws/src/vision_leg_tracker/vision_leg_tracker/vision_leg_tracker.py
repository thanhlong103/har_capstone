...
# should implement the custom_message
from leg_detector_msgs.msg import PersonArray, Person

# Offset (mm) must be obtained from measurment
# These obset values are not fixed since the final structure of the robot
# is not formed
offset_x = 10.0
offset_y = 250.0
offset_z = 0.0
...
class VisionTrackerNode(Node):
    def __init__(self):
        ...
        self.people_pub = self.create_publisher(
            PersonArray, "/people_detected_vision", 10)

        self.interpreter = tf.lite.Interpreter(
            model_path='/home/har_capstone/vision_leg_tracker_ws/src/vision_leg_tracker/vision_leg_tracker/model.tflite')
        ...
        try:
            ...
            self.image_pub.publish(
                self.bridge.cv2_to_imgmsg(
                    estimated_frame, "bgr8"))

            legs_world = self.localization(
                offset, intrinsics, legs_pixel, depth_array)
